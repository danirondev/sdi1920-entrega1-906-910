...
//		
	...
	@Test
	public void PR20() {
		String ingles = "btnEnglish";
		String español = "btnSpanish";
		//Comprobamos que estamos en la pagina de bienvenida
		PO_HomeView.checkWelcome(driver, PO_Properties.getSPANISH()); 
		PO_HomeView.checkElement(driver, "text", "Bienvenidos a la pagina principal");
		//Cambiamos idioma a inglés
		PO_HomeView.changeIdiom(driver, ingles);
		//Comprobamos el texto en inglés
		PO_HomeView.checkElement(driver, "text", "Welcome to homepage");
		//Volvemos a español
		PO_HomeView.changeIdiom(driver, español);
		
		//Vamos al formulario de identificación
		PO_HomeView.clickOption(driver, "login", "class", "btn btn-primary");
		//Comprobamos que estamos en la vista del formulario en español
		PO_HomeView.checkElement(driver, "text", "Identifícate");
		//Cambiamos idioma a inglés
		PO_HomeView.changeIdiom(driver, ingles);
		//Comprobamos el texto en inglés
		PO_HomeView.checkElement(driver, "text", "Identified yourself");
		//Volvemos a español
		PO_HomeView.changeIdiom(driver, español);
		
		//Rellenamos el formulario.
		PO_LoginView.fillForm(driver, "pedro@email.com", "123456");
		//Comprobamos que entramos en la seccion privada en español
		PO_View.checkElement(driver, "text", "Esta zona solo es visible para ti");
		//Cambiamos idioma a inglés
		PO_HomeView.changeIdiom(driver, ingles);
		//Comprobamos el texto en inglés
		PO_HomeView.checkElement(driver, "text", "This area can only be visible by you");
		//Volvemos a español
		PO_HomeView.changeIdiom(driver, español);
		
		//Vamos a la vista de amistades
		PO_PrivateView.goToFriends(driver, "http://localhost:8090/friend/list");
		//Comprobamos que estamos en la vista de amigos en español
		PO_HomeView.checkElement(driver, "text", "Amistades");
		//Cambiamos idioma a inglés
		PO_HomeView.changeIdiom(driver, ingles);
		//Comprobamos el texto en inglés
		PO_HomeView.checkElement(driver, "text", "Friends");
		//Volvemos a español
		PO_HomeView.changeIdiom(driver, español);
		
		//Clickamos en el boton de desloggearnos.
		PO_HomeView.clickOption(driver, "logout", "class", "btn btn-primary");		
	}
	
	//PR21. Intentar acceder sin estar autenticado a la opción de listado de usuarios. Se deberá volver al formulario de login. 
	@Test
	public void PR21() {
		//Sin autentificarnos, intentamos acceder a la página de listar usuarios.
		PO_PrivateView.goToUsers(driver, "http://localhost:8090/user/list");
		//Comprobamos que estamos en la vista de login
		PO_HomeView.checkElement(driver, "text", "Identifícate");
	}
	
	//PR22. Intentar acceder sin estar autenticado a la opción de listado de amigos(ya que no
	//hemos implementado los opcionales con las publicaciones). Se deberá volver al formulario de login. 
	@Test
	public void PR22() {
		//Sin autentificarnos, intentamos acceder a la página de listar amigos.
		PO_PrivateView.goToUsers(driver, "http://localhost:8090/friend/list");
		//Comprobamos que estamos en la vista de login
		PO_HomeView.checkElement(driver, "text", "Identifícate");
	}
	
	//PR23. Estando autenticado como usuario estándar intentar acceder a una opción disponible solo para usuarios 
	//administradores (Se puede añadir una opción cualquiera en el menú).  Se deberá indicar un mensaje de acción prohibida. 
	@Test
	public void PR23() {
		//Vamos al formulario de identificación
		PO_HomeView.clickOption(driver, "login", "class", "btn btn-primary");
		...
		//Comprobamos que entramos en la seccion privada
		PO_View.checkElement(driver, "text", "Esta zona solo es visible para ti");
		//Intentamos acceder a la vista privilegiada del administrador
		PO_PrivateView.goToAdminView(driver, "http://localhost:8090/user/adminView");
		//Comprobamos que salta un mensaje prohibitibo (HTTP Status 403 – Forbidden)
		SeleniumUtils.EsperaCargaPaginaNoTexto(driver, "ERES EL ADMINISTRADOR DE LA APLICACIÓN!!", PO_View.getTimeout());
	}
